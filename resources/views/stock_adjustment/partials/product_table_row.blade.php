<tr class="product_row">
    <td>
        ...
        @if($product->enable_sr_no == 1)
            <div>
                <input type="hidden" id="product_id{{$row_index}}" value="{{ $product->product_id }}">
                <a id="{{ $row_index }}" class="create-sell-serials" data-toggle="tooltip" data-placement="bottom" title="Add Serials" href="javascript:void(0)"><i class="fa fa-barcode"></i></a>
            </div>
        @endif
    ...
    <td>
        ...
        <input type="text" class="form-control product_quantity input_number input_quantity{{ $row_index }}" value="{{@format_quantity($product->quantity_ordered)}}" name="products[{{$row_index}}][quantity]"
        ...
        data-msg_max_default="@lang('validation.custom-messages.quantity_not_available', ['qty'=> $product->formatted_qty_available, 'unit' => $product->unit  ])"
         @endif >
        {{$product->unit}}
        <input type="hidden" id="serial_numbers" name="products[{{$row_index}}][serial_numbers]" value="">
