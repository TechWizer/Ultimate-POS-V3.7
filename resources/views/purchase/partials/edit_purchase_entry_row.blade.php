    ...
    @foreach($purchase->purchase_lines as $purchase_line)
        <tr>
            ...
            <td>
                ...
                ['class' => 'form-control input-sm purchase_quantity input_quantity'.$loop->index.' mousetrap', 'required', 'data-rule-abs_digit' => $check_decimal, 'data-msg-abs_digit' => __('lang_v1.decimal_value_not_allowed')]); !!}
            ...
            <td>
                {!! Form::text('purchases[' . $loop->index . '][free_issue]',
                number_format($purchase_line->free_issues, $quantity_precision, $currency_details->decimal_separator, $currency_details->thousand_separator),
                ['class' => 'form-control input-sm input_number mousetrap', 'required', 'data-rule-abs_digit' => $check_decimal, 'data-msg-abs_digit' => __('lang_v1.decimal_value_not_allowed')]); !!}
            ...
            @if($purchase_line->product->enable_sr_no == 1)
                @if(!empty($purchase_line->serials))
                    <td>
                        <button value="{{ $purchase_line->id }}" type="button" class="btn btn-primary btn-xs purchased-serials">Purchased Serials</button>
                    </td>
                @else
                    <td>&nbsp;</td>
                @endif
                <td>
                    <input type="hidden" id="product_id" value="{{ $purchase_line->product_id }}">
                    <input type="hidden" id="serial_numbers{{ $loop->index }}" name="purchases[{{$loop->index}}][serial_numbers]" value="">
                    <input type="hidden" name="purchases[{{$loop->index}}][purchased_serial_numbers]" value="{{ $purchase_line->serials }}">
                    <button value="{{ $loop->index }}" type="button" class="btn btn-primary btn-xs add_serials_row"><i class="fa fa-plus"></i> Serials</button>
                </td>
            @else
                <td>&nbsp;</td>
                <td>&nbsp;</td>
