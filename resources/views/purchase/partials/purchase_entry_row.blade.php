@foreach( $variations as $variation)
    <tr>
        ...
        <td>
            {!! Form::text('purchases[' . $row_count . '][free_issue]', number_format(0, $quantity_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'form-control input-sm input_number mousetrap', 'required', 'data-rule-abs_digit' => $check_decimal, 'data-msg-abs_digit' => __('lang_v1.decimal_value_not_allowed')]); !!}
        ...
        @php
            //get the prevoius url and check
                $name = str_replace(url('/'), '', url()->previous());
                $names = explode('/', $name);
        @endphp
        @if($product->enable_sr_no == 1)
            @if(!empty($names[3]) == 'edit')
                <td>&nbsp;</td>
            @endif
            <td>
                <input type="hidden" id="product_id" value="{{ $product->id }}">
                <input type="hidden" id="serial_numbers{{ $row_count }}" name="purchases[{{$row_count}}][serial_numbers]" value="">
                <button value="{{ $row_count }}" type="button" class="btn btn-primary btn-xs add_serials_row"><i class="fa fa-plus"></i> Serials</button>
            </td>
        @else
            @if(!empty($names[3]) == 'edit')
                <td>&nbsp;</td>
