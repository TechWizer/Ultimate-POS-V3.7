        ...
        <div class="row">
            ...
            <div class="@if(!empty($default_purchase_status)) col-sm-4 @else col-sm-3 @endif">
              <div class="form-group">
                ...
                {!! Form::text('ref_no', $purchase->ref_no, ['class' => 'form-control', 'style'=>'display:none']); !!}
              </div>
            </div>
...
{{--purchased serials show--}}
<div class="modal fade" id="sold-purchased-serials-modal"></div><!-- /.modal -->
@endsection
...
@section('javascript')
  ...
  <script type="text/javascript">
    $(document).ready( function(){
      ...
      // purchased serials show
      $(document).on('click', '.purchased-serials', function () {
        let purchase_line_id = $(this).val();
        let url = '{{ route('purchased-serial-show', ['id', 'type' => 'purchased']) }}';
        url = url.replace('id', purchase_line_id);
        $('#sold-purchased-serials-modal').load(url, function () {
            $('#example1').DataTable();
            $('#sold-purchased-serials-modal').modal('show');
        });
      });
