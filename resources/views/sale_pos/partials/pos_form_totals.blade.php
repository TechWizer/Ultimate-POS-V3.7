				...
				<td>
					<b>
						...
						<i class="fas fa-edit cursor-pointer" id="pos-edit-discount" title="@lang('sale.edit_discount')" aria-hidden="true" data-toggle="modal" data-target="#posEditDiscountModal"></i>
							...
							<input type="hidden" name="coin_points_earned_amount" id="coin_points_earned_amount" value="@if(empty($edit)){{'0'}}@else {{$transaction->coin_points_earned_amount}} @endif">
			    			<input type="hidden" name="coin_points_redeemed_amount" id="coin_points_redeemed_amount" value="@if(empty($edit)){{'0'}}@else {{$transaction->coin_points_redeemed_amount}} @endif">
