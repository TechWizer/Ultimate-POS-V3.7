<tr class="product_row" data-row_index="{{$row_count}}">
	<td>
		...
		@if($product->enable_sr_no == 1)
			<div>
				<input type="hidden" id="product_id{{$row_count}}" value="{{ $product->product_id }}">
				<a id="{{ $row_count }}" class="create-sell-serials" data-toggle="tooltip" data-placement="bottom" title="Add Serials" href="javascript:void(0)"><i class="fa fa-barcode"></i></a>
				@if(!empty($product->sold_serials))
				<a id="{{ $product->transaction_sell_lines_id }}" class="sold-sell-serials" data-toggle="tooltip" data-placement="bottom" title="Sold Serials" href="javascript:void(0)"><i class="fa fa-archive"></i></a>
				@endif
			</div>
		@endif
		@php
	...
	<td>
		...
		<input type="text" data-min="1" 
			class="form-control pos_quantity input_number mousetrap input_quantity{{$row_count}}"
		...
		<input type="hidden" id="serial_numbers{{ $row_count }}" name="products[{{$row_count}}][serial_numbers]" value="{{ $product->sold_serials }}">
