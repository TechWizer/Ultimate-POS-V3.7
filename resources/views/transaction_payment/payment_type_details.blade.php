...
<div class="payment_details_div @if( $payment_line->method !== 'cheque' ) {{ 'hide' }} @endif" data-type="cheque" >
	<div class="col-md-6">
		<div class="form-group">
			...
			{!! Form::text("cheque_number", !empty($payment_line->cheque)?$payment_line->cheque->cheque_number:'', ['class' => 'form-control', 'placeholder' => __('lang_v1.cheque_no')]); !!}
		</div>
	</div>
	<div class="col-md-6">
		<div class="form-group">
			...
			<div class="input-group">
              <span class="input-group-addon">
				...
				{!! Form::text('cheque_issued_date', @format_datetime(!empty($payment_line->cheque)?$payment_line->cheque->cheque_issued_date: now()), ['class' => 'form-control', 'readonly', 'required', 'id' => 'cheque_issued_date']); !!}
			</div>
		</div>
	</div>
	<div class="col-md-6">
		<div class="form-group">
			...
			<select class="form-control select2" style="width: 100%;" name="cheque_type" required>
				<option value="giving" {{ !empty($payment_line->cheque)?$payment_line->cheque->cheque_type == 'giving'?'selected':'':'' }}>Issued</option>
				<option value="receiving" {{ !empty($payment_line->cheque)?$payment_line->cheque->cheque_type == 'receiving'?'selected':'':'' }}>Received</option>
			</select>
		</div>
		<!-- /.form-group -->
	</div>
