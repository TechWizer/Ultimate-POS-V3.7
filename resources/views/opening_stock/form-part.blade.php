...
@foreach($purchases[$key][$variation->id] as $sub_key => $var)
	...
	$added_serials = $var['added_serials'];
	@endphp
...
<tr>
	...
	<td>
		<div class="input-group">
		  {!! Form::text('stocks[' . $key . '][' . $variation->id . '][' . $sub_key . '][quantity]', @format_quantity($qty) , ['class' => 'form-control input-sm input_number purchase_quantity input_quantity'.$sub_key, 'required']); !!}
...
@endif
	...
	@if($product->enable_sr_no == 1)
		<td>
			<input type="hidden" id="serial_numbers{{$sub_key}}" name="stocks[{{$key}}][{{$variation->id}}][{{$sub_key}}][serial_numbers]" value="">
			<input type="hidden" name="stocks[{{$key}}][{{$variation->id}}][{{$sub_key}}][added_serial_numbers]" value="{{ $added_serials }}">
			<button value="{{ $sub_key }}" type="button" class="btn btn-primary btn-xs add_serials_row"><i class="fa fa-plus"></i> Add Serials</button>
		</td>
	@endif
	<td>
		@if($loop->index == 0)
			<button type="button" class="btn btn-primary btn-xs add_stock_row" data-sub-key="{{ count($purchases[$key][$variation->id])}}" 
				data-row-html='<tr>
					...
					<td>
					<div class="input-group">
	              		<input class="form-control input-sm input_number purchase_quantity input_quantity__subkey__" required="" name="stocks[{{$key}}][{{$variation->id}}][__subkey__][quantity]" type="text" value="0">
			              <span class="input-group-addon">
			                {{ $product->unit->short_name }}
			              </span>
	        			</div>
	...
	@if($product->enable_sr_no == 1)
						<td>
							<input type="hidden" id="serial_numbers__subkey__" name="stocks[{{$key}}][{{$variation->id}}][__subkey__][serial_numbers]" value="">
							<button value="__subkey__" type="button" class="btn btn-primary btn-xs add_serials_row"><i class="fa fa-plus"></i> Add Serials</button>
						</td>
@endif
	<td>&nbsp;</td></tr>'
