...
@endif
@if (!empty($pos_settings['enable_coin_points']))
	<input type="hidden" id="enable_coin_points">
@endif
	...
	<div class="row">
		<div class="col-md-12 col-sm-12">
			...
			@component('components.widget', ['class' => 'box-primary'])
				...
				<div class="col-md-12 well well-sm bg-light-gray @if(empty($pos_settings['enable_coin_points'])) hide @endif">
					@if (!empty($pos_settings['enable_coin_points']))
						<input type="hidden" name="enable_coin_points" value="1">
						@else
						<input type="hidden" name="enable_coin_points" value="0">
					@endif
			    	<input type="hidden" name="coin_points_earned_amount" id="coin_points_earned_amount" value="{{ $transaction->coin_points_earned_amount }}">
			    	<input type="hidden" name="coin_points_redeemed_amount" id="coin_points_redeemed_amount" value="{{ $transaction->coin_points_redeemed_amount }}">
			    	{{-- <div class="col-md-12"><h4>{{session('business.rp_name')}}</h4></div> --}}
			    	<div class="col-md-4">
				        <div class="form-group">
				            {!! Form::label('coin_points_redeemed_amount_modal', __('lang_v1.coin_points_redeemed') . ':' ) !!}
				            <div class="input-group">
				                <span class="input-group-addon">
				                    <i class="fa fa-gift"></i>
				                </span>
				                {!! Form::number('coin_points_redeemed_amount_modal', $transaction->coin_points_redeemed_amount , ['class' => 'form-control direct_sell_coin_points_input', 'data-max_points' => !empty($reward_points_details['total_coins']) ? $reward_points_details['total_coins'] : 0]); !!}
				                {{-- <input type="hidden" id="rp_name" value="{{session('business.rp_name')}}"> --}}
				            </div>
				        </div>
				    </div>
				    <div class="col-md-4">
				    	<p><strong>@lang('lang_v1.available_coin_points'):</strong> <span id="available_coin_points">{{ $reward_points_details['total_coins'] ?? 0 }}</span></p>
				    </div>
			    </div>
			    ...
			    <div class="col-md-4 col-md-offset-8">
			    	...
			    	<div><b>@lang('sale.total_payable'): </b>
						...
						<input type="hidden" id="old_final_total_input" value=0>
...
{{--        serial create modal--}}
<div class="modal fade" id="serial-sell-create-modal"></div><!-- /.modal -->
{{--sold serials show--}}
<div class="modal fade" id="sold-serials-modal"></div><!-- /.modal -->
<!-- quick product modal -->
...
@section('javascript')
	...
	<script>
		$(document).ready(function(){
			$(document).on('click', '.sold-sell-serials', function () {
				let transaction_sell_line_id = $(this).attr('id');
				let url = '{{ route('sold-serial-show', ['id', 'type' => 'sell']) }}';
				url = url.replace('id', transaction_sell_line_id);
				$('#sold-serials-modal').load(url, function () {
					$('#example1').DataTable();
					$('#sold-serials-modal').modal('show');
				});
			})
		});
	</script>
