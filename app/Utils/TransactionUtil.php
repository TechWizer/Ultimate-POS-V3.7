    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                $this->editSellLine($product, $location_id, $status_before, $multiplier, $transaction->id);
                ...
                $line = [
                    ...
                    'sold_serials' => !empty($product['serial_numbers']) ? $product['serial_numbers'] : null
                ];
...
//            get the created transaction sell line
                if (!empty($product['serial_numbers'])) {
                    $serial_numbers = $product['serial_numbers'];
                    $serials = explode(',', $serial_numbers);
                    foreach ($serials as $serial) {
                        $variation_location_serial = VariationLocationSerial::where('serial_number', $serial)->first();
                        $variation_location_serial->update([
                            'status' => 'sold',
                            'invoice_no_transaction_id' => $transaction->id
                        ]);
                    }
                }
     ...
     * @return boolean
     */
    public function editSellLine($product, $location_id, $status_before, $multiplier = 1, $transaction_id)
    {
        ...
        //        updating variation location serials
        if (!empty($product['serial_numbers'])) {
            $sold_serials = $sell_line->sold_serials;
            $serials = explode(',', $sold_serials);
            foreach ($serials as $serial) {
                $variation_location_serial = VariationLocationSerial::where('serial_number', $serial)->first();
                $variation_location_serial->update([
                    'status' => 'available',
                    'invoice_no_transaction_id' => null
                ]);
            }
            ...
            foreach ($serials as $serial) {
                ...
                $variation_location_serial->update([
                    'status' => 'sold',
                    'invoice_no_transaction_id' => $transaction_id
                ]);
            }
        }
        ...
        if (!empty($transaction_line_ids)) {
            ...
            if ($adjust_qty) {
                foreach ($sell_lines as $line) {
                    ...
                    $sold_serials = $line->sold_serials;
                    if (!empty($sold_serials)) {
                        $serials = explode(',', $sold_serials);
                        foreach ($serials as $serial) {
                            $variation_location_serial = VariationLocationSerial::where('serial_number', $serial)->first();
                            if (!empty($variation_location_serial)) {
                                $variation_location_serial->update([
                                    'status' => 'available',
                                    'invoice_no_transaction_id' => null
                                ]);
    ...
    public function createOrUpdatePaymentLines($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true)
    {
        ...
        foreach ($payments as $payment) {
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($transaction->coin_points_earned_amount !== 0 && $flag) {
                    $payment_amount -= $transaction->coin_points_earned_amount;
                    $flag = false;
                }
...
//                            $payment_amount = $amount + $added_value;
                            $payment['note'] = "Card percentage payment added :" . $added_value;
                        }
...
//                            'card_holder_name' => isset($payment['card_holder_name']) ? $payment['card_holder_name'] : null,
    ...
    public function editPaymentLine($payment, $transaction = null, $uf_data = true)
    {
        ...
        unset($payment['transaction_no_1'], $payment['transaction_no_2'], $payment['transaction_no_3'],
            $payment['cheque_issued_date'], $payment['cheque_type'], $payment['cheque_date']);
            ...
            foreach ($due_transactions as $transaction) {
                ...
                if ($total_amount > 0) {
                    ...
                    $array = [
                        ...
                        'note' => !empty($parent_payment->note)?$parent_payment->note:'',
                        'account_id' => !empty($parent_payment->account_id) ? $parent_payment->account_id : null,
    ...
    public function getGrossProfit($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null)
    {
        ...
        $stock_disabled_product_sell_details =
            $query_2->select(DB::raw('SUM( 
                        (transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned ) * (transaction_sell_lines.unit_price_inc_tax - variations.dpp_inc_tax) ) as gross_profit'))
                ->first();
    ...
    public function updateCustomerCoinPoints($customer_id, $coin_points_earned_amount, $coin_points_earned_before = 0,  $coin_points_redeemed_amount = 0, $coin_points_redeemed_before = 0)
    {
        $customer = Contact::findOrFail($customer_id);
        $total_earned = $coin_points_earned_amount - $coin_points_earned_before;
        $total_redeemed = $coin_points_redeemed_amount - $coin_points_redeemed_before;

        $difference = $total_earned - $total_redeemed;
        $customer_coins = empty($customer->total_coins) ? 0 : $customer->total_coins;
        $total_coins = $customer_coins + $difference;
        $customer->total_coins = $total_coins;
        $customer->total_coins_used += $total_redeemed;
        $customer->update();
    }
    ...
    public function getCustomerCoinPoints($business_id, $customer_id)
    {
        try {
            if (session()->has('business')) {
                $business = session()->get('business');
            ...
            $coin_points = Contact::where([['id', $customer_id], ['business_id', $business_id]])
            ->select(
                'total_coins',
                // 'total_coins_used'
            )
    ...
    public function deleteSale($business_id, $transaction_id)
    {
        ...
        if (!empty($transaction)) {
            ...
            if ($transaction->status == 'draft') {
                ...
                $this->updateCustomerCoinPoints($transaction->contact_id, 0, $transaction->coin_points_earned_amount, 0, $transaction->coin_points_redeemed_amount);
    ...
    public function addSellReturn($input, $business_id, $user_id, $uf_number = true)
    {
        ...
        foreach ($sell->sell_lines as $sell_line) {
            if (array_key_exists($sell_line->id, $returns)) {
                ...
                $productUtil->updateProductQuantity($sell_return->location_id, $sell_line->product_id, $sell_line->variation_id, 
                    $quantity, $quantity_before, null, false, null, null, null, true, null);
            }
        }
    ...
    // API functions
    public function createOrUpdatePaymentLinesAPI($transaction, $payments, $business_id, $user_id, $uf_data = true)
    {
        $payments_formatted = [];
        $edit_ids = [0];
        $account_transactions = [];
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    if ($payment['method'] != 'cheque') {
                        ...
                        $default_account = Account::where('is_default', true)->select('id')->first();
                        if (!empty($default_account)) {
                            //                    if default bank account exists
                            $payment['account_id'] = $default_account->id;
                        }
                        ...
                        //                        cheque create
                        $cheque_inputs['cheque_number'] = $payment['cheque_number'];
                        $cheque_inputs['cheque_date'] = Carbon::parse($payment['card_year'])->format('Y-m-d h:m:s');
                        $cheque_inputs['cheque_issued_date'] = Carbon::parse(now())->format('Y-m-d h:m:s');
                        $cheque_inputs['cheque_amount'] = $payment_amount;
                        $cheque_inputs['cheque_status'] = 'due';
                        $cheque_inputs['cheque_type'] = 'receiving';
                        $cheque_inputs['account_id'] = $payment['account_id'];
        ...
        if (!empty($payments_formatted)) {
            ...
            foreach ($transaction->payment_lines as $key => $value) {
                ...
                if (!empty($value->account_id)) {
                    ...
                    $account_transaction_inputs['account_id'] = $value->account_id;
                    $account_transaction_inputs['type'] = $type;
                    $account_transaction_inputs['amount'] = $value->amount;
                    $account_transaction_inputs['operation_date'] = now();
                    $account_transaction_inputs['created_by'] = $user_id;
                    $account_transaction_inputs['transaction_id'] = $transaction->id;
                    $account_transaction_inputs['transaction_payment_id'] = $value->id;
                    $account_transaction_inputs['note'] = '';
