...
//           check if product variation exists
            $product_variation = ProductVariation::where('variation_template_id', $variation_template_id)->where('product_id', $product->id)->first();
            ...
            if (!empty($value['variations'])) {
                ...
                foreach ($value['variations'] as $k => $v) {
                    ...
                    $dss = $this->num_uf($v['default_sell_price']);
                    $sell_price_new = round($dss);
     ...
     * @return boolean
     */
    public function updateProductQuantity($location_id, $product_id, $variation_id, $new_quantity, $old_quantity = 0, $number_format = null, $uf_data = true, $serial_numbers = null, $purchased_serial_numbers = null, $transaction_id = null,$new_variation = null, $new_variation_id = null)
    {
        ...
        if ($product->enable_stock == 1 && $qty_difference != 0) {
            ...
            }elseif (!is_null($new_variation) &&  $new_variation == false) {
                $variation_location_d = new VariationLocationDetails();
                $variation_location_d->variation_id = $new_variation_id;
                $variation_location_d->product_id = $product_id;
                $variation_location_d->location_id = $location_id;
                $variation_location_d->product_variation_id = $variation->product_variation_id;
            ...
            if (!is_null($new_variation) && $new_variation == true) {
                $variation_location_d->qty_available += $qty_difference;
            }
            $variation_location_d->save();
            $variation_location_id = $variation_location_d->id;
        }

//        purchased serials

        if (!empty($serial_numbers) && !empty($purchased_serial_numbers)) {
            $serials = explode(',', $purchased_serial_numbers);
            foreach ($serials as $serial) {
                VariationLocationSerial::where('serial_number', $serial)->delete();
            }
        }
...
//                create serials
            foreach ($serials as $serial) {
                VariationLocationSerial::create([
                    'variation_location_detail_id' => $variation_location_id,
                    'product_id' => $product_id,
                    'serial_number' => $serial,
                    'status' => 'available',
                    'ref_no_transaction_id' => $transaction_id
                ]);
    ...
    // this is add me for puchase status change
    public function updateProductQuantityViaPuchaseStatus($location_id, $product_id, $variation_id, $new_quantity, $old_quantity = 0, $number_format = null, $uf_data = true, $serial_numbers = null, $purchased_serial_numbers = null, $transaction_id = null,$new_variation = null, $new_variation_id = null)
    {
...
//                create serials
    ...
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true)
    {
        ...
        $product = $query->select(
            ...
            'variations.sell_code',
            'variations.cost_code',
            ...
            DB::raw("IF(p.enable_stock=0, variations.dpp_inc_tax, (SELECT purchase_price_inc_tax FROM purchase_lines WHERE 
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            $purchased_serials_numbers = null;
            if (!empty($data['purchased_serial_numbers'])) {
                $purchased_serials_numbers = $data['purchased_serial_numbers'];
            }
...
//           check purchase price and sell price are equals
            ...
            if (isset($data['purchase_line_id'])) {
                ...
                $this->updateProductStock($before_status, $transaction, $data['product_id'], $data['variation_id'], $new_quantity, $purchase_line->quantity, $currency_details, $serial_numbers, $purchased_serials_numbers,  $new_variation, $new_variation_id);
            } else {
                ...
                if ($transaction->status == 'received') {
                    $this->updateProductQuantity($transaction->location_id, $data['product_id'], $data['variation_id'], $new_quantity_f, 0, $currency_details, false, $serial_numbers, null, $transaction->id, $new_variation, $new_variation_id);
                }
            }

            $purchase_line->quantity = $new_quantity;
            $purchase_line->free_issues = $this->num_uf($data['free_issue']);
            ...
            $purchase_line->serials = !empty($serial_numbers) ? $serial_numbers : $purchase_line->serials;
    ...
    public function createNewVariableProductVariations($product_type, $variation, $data, $new_variation, $new_variation_id, $index): array
    {
//                    create new price batch if cost price or selling price changed
        ...
        $latest_variation_value_template_name = $latest_variation_value_template_name . $index;
        ...
        $input_variations[0]['variations'][0] = [
            ...
            'sell_price_inc_tax' => $data['default_sell_price']
        ];
//                    dd($input_variations);
     ...
     *
     */
    public function updateProductStock($status_before, $transaction, $product_id, $variation_id, $new_quantity, $old_quantity, $currency_details, $serial_numbers = null, $purchased_serials_numbers = null, $new_variation = null, $new_variation_id = null)
    {
        $new_quantity_f = $this->num_f($new_quantity);
        ...
        if ($status_before == 'received' && $transaction->status == 'received') {
            ...
            $this->updateProductQuantity($transaction->location_id, $product_id, $variation_id, $new_quantity_f, $old_qty, $currency_details, false, $serial_numbers, $purchased_serials_numbers, $transaction->id,  $new_variation, $new_variation_id);
            ...
            $this->updateProductQuantity($transaction->location_id, $product_id, $variation_id, $new_quantity_f, 0, $currency_details, false, $serial_numbers, $purchased_serials_numbers, $transaction->id,  $new_variation, $new_variation_id);
        }
    }

    // this is add me for puchase status change
    public function updateStockViaPurchaseStatus($status_before, $transaction, $product_id, $variation_id, $new_quantity, $old_quantity, $currency_details, $serial_numbers = null, $purchased_serials_numbers = null, $new_variation = null, $new_variation_id = null)
    {
        $new_quantity_f = $this->num_f($new_quantity);
        $old_qty = $this->num_f($old_quantity);
        //Update quantity for existing products
        if ($status_before == 'received' && $transaction->status == 'received') {
            ...
            $this->updateProductQuantityViaPuchaseStatus($transaction->location_id, $product_id, $variation_id, $new_quantity_f, 0, $currency_details, false, $serial_numbers, $purchased_serials_numbers, $transaction->id,  $new_variation, $new_variation_id);
        }
    }
    ...
    public function filterProduct($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        ...
        $query->select(
            ...
            'variations.default_purchase_price as cost_price',
            'variations.sub_sku',
            'U.short_name as unit'
        );
