...
class PurchaseReturnController extends Controller
{
    ...
    public function getSuppliersPurchaseInvoices($id) {
            if (\request()->ajax()){
                $business_id = request()->session()->get('user.business_id');
                $purchases = Transaction::leftJoin('transaction_payments', 'transaction_payments.transaction_id', '=', 'transactions.id')
                    ->where('transactions.business_id', $business_id)
                ...
                foreach ($purchases as $purchase) {
                    $paid_amount = !empty($purchase->paid_amount)?$purchase->paid_amount:0;
                    $due_amount = $purchase->final_total - $paid_amount;
                    $html .= '<option value="'.$purchase->id.'">'.$purchase->ref_no.' - Rs.'. number_format($due_amount, 2) .'</option>';
                }
                return $html;
            }
    }

    public function getPurchaseRows(Request $request) {
        if ($request->ajax()){
            $business_id = request()->session()->get('user.business_id');
            ...
            if (!empty($invoice_ids)){
                foreach ($invoice_ids as $key => $invoice_id) {
                    $purchases = Transaction::leftJoin('transaction_payments', 'transaction_payments.transaction_id', '=', 'transactions.id')
                        ...
                        ->where([['transactions.business_id', $business_id], ['type', 'purchase'], ['transactions.id', $invoice_id]])
                        ->select(
                            'transactions.id',
                            'transactions.ref_no',
                            'transactions.final_total',
                    ...
                    // $paid_amount = !empty($purchases->amount)?$purchases->amount:0;
                    $due_amount = $purchases->final_total - $purchases->paid_amount;
                    $html .= '<tr>
                            <td>
                                <input type="hidden" name="purchase_invoices['.$purchases->id.'][id]" value="'.$purchases->id.'">
                                <input type="hidden" name="purchase_invoices['.$purchases->id.'][ref_no]" value="'.$purchases->ref_no.'">
                                '.$purchases->ref_no.'
                            </td>
                            <td>
                                <input type="hidden" id="final_total'.$purchases->id.'" name="purchase_invoices['.$purchases->id.'][final_total]" value="'.$due_amount.'" >Rs.'.number_format($due_amount, 2).'
                            </td>
                            <td>
                                <div class="form-group">
                                    <input type="text" class="form-control purchase_invoice_pay_amount" id="'.$purchases->id.'" name="purchase_invoices['.$purchases->id.'][pay_amount]" value="0" placeholder="Input Amount">
                                </div>
                            </td>
