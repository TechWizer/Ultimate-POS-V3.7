...
class ImportOpeningStockController extends Controller
{
    ...
    private function addOpeningStock($opening_stock, $product, $business_id, $unit_cost_before_tax, $transaction = null)
    {
        ...
        $this->productUtil->updateProductQuantity($opening_stock['location_id'], $product->id, $product->variation_id,
                                $opening_stock['quantity'], 0, null, false, null, null, null, true, null);
    }
    public function updatePurchaseLinesQuantityToVariationLocationDetailsQuantityAvailable()
    {
        try {
            ...
            foreach($purchase_lines as $purchase_line) {
                ...
                $variation_location_detail = VariationLocationDetails::where([['product_id', $purchase_line->product_id], 
                    ['variation_id', $purchase_line->variation_id]])->first();
                    if($purchase_line->product_id == 6792)
                  
                    if(!empty( $variation_location_detail)){
                $variation_location_detail->qty_available = $purchase_line->quantity;
                $variation_location_detail->update();
                    }
            }
            DB::commit();
            $output = [
