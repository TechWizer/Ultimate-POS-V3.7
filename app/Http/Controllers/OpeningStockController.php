        ...
        if (!empty($product) && $product->enable_stock == 1) {
            ...
            foreach ($transactions as $transaction) {
                ...
                foreach ($transaction->purchase_lines as $purchase_line) {
                    ...
                    $purchase_lines[$purchase_line->variation_id][$k]['added_serials'] = $purchase_line->serials;
                }
                    ...
                    if (array_key_exists($location_id, $locations)) {
                        ...
                        foreach ($value as $k => $rows) {
                            foreach ($rows as $key => $v) {
                                ...
                                $serial_numbers = null;
                                if (!empty($v['serial_numbers'])){
                                    $serial_numbers = $v['serial_numbers'];
                                }

                                $added_serial_numbers = null;
                                if (!empty($v['added_serial_numbers'])){
                                    $added_serial_numbers = $v['added_serial_numbers'];
                                }
                                ...
                                if (isset($v['purchase_line_id'])) {
                                    ...
                                    if ($qty_remaining != 0) {
                                        ...
                                        $this->productUtil->updateProductQuantity($location_id, $product->id, $k, $qty_remaining, $old_qty, null, false, 
                                            $serial_numbers, $added_serial_numbers, null, true, null);
                                    }
                                } else {
                                    if ($qty_remaining != 0) {
