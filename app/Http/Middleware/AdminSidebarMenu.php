            ...
            if (auth()->user()->can('product.view') || auth()->user()->can('product.create') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        $sub->url(
                            action('SerialController@index'),
                            'Serial Report',
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'serial' && request()->segment(2) == '']
                        );
                        ...
                        if (auth()->user()->can('brand.view') || auth()->user()->can('brand.create')) {
                            $sub->url(
                                action('ManufactureController@index'),
                                __('Manufactures'),
                                ['icon' => 'fa fas fa-industry', 'active' => request()->segment(1) == 'manufactures']
                            );
                        }
                        if (auth()->user()->can('brand.view') || auth()->user()->can('brand.create')) {
                            $sub->url(
                                action('TypeController@index'),
                                __('Types'),
                                ['icon' => 'fa fas fa-certificate', 'active' => request()->segment(1) == 'types']
                            );
                        }
                        if (auth()->user()->can('brand.view') || auth()->user()->can('brand.create')) {
                            $sub->url(
                                action('VehicleModelController@index'),
                                __('Models'),
                                ['icon' => 'fa fas fa-car', 'active' => request()->segment(1) == 'models']
                            );
                        }
            ...
            if (auth()->user()->can('account.access') && in_array('account', $enabled_modules)) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        $sub->url(
                            action('BulkPaymentController@create'),
                            __('account.bulk_payment'),
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'account' && request()->segment(2) == 'bulk-payment']
                        );
                        $sub->url(
                    ...
                    function ($sub) use ($enabled_modules) {
                        ...
                        if (auth()->user()->can('purchase_n_sell_report.view')) {
                            ...
                            $sub->url(
                                ...
                                __('lang_v1.audit_logs'),
                                ['icon' => 'fa fas fa-arrow-circle-down', 'active' => request()->segment(2) == 'audit-logs']
                            );
