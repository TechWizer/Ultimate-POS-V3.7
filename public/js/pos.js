$(document).ready(function() {
    ...
    if ($('#search_product').length) {
        ...
        $('#search_product')
            ...
            if (item.enable_stock == 1 && item.qty_available <= 0 && !is_overselling_allowed) {
                ...
                let user = $('#auth_user').val();
                if (user === 'admin'){
                    string += ' - Cost Price :'+item.cost_price;
                }
                string += '</div>';
    ...
    $('button#pos-finalize').click(function() {
        ...
        if ($('#enable_coin_points').length) {
            var validate_rp = isValidCoinPoint();
            if (!validate_rp['is_valid']) {
                toastr.error(validate_rp['msg']);
                return false;
            }
    ...
    $('button#pos-save-card').click(function() {
        ...
        let final_total = __number_uf($('#old_final_total_input').val());
        let percentage = $('#card_holder_name').val();
        let added_amount = (final_total * percentage) / 100;
        let added_final_total = final_total + added_amount;
        $('.payment-amount').val(__number_f(added_final_total));
        $('#final_total_input').val(__number_f(added_final_total));
        // console.log('final_total : '+final_total+" percentage : "+percentage+" added amount : "+added_amount+" added final amount : "+added_final_total);
    ...
    $(document).on('change', '.card_holder_name', function () {
       calculate_balance_due();
    })
    ...
    $('button#posEditDiscountModalUpdate').click(function() {
        ...
        if ($('#enable_coin_points').length) {
            var reward_validation = isValidCoinPoint();
            if (!reward_validation['is_valid']) {
                toastr.error(reward_validation['msg']);
                $('#coin_points_redeemed_amount_modal').val(0);
                $('#coin_points_redeemed_amount_modal').change();
            }
            updateCoinPointsAmount();
        }
    ...
    $('button#submit-sell, button#save-and-print').click(function(e) {
        ...
        if (sell_form.valid()) {
            ...
            if (customer_id !== '1' && $('#enable_coin_points').length) {
                addCoinPoints();
            } else {
                sell_form.submit();
            }
    ...
    function addCoinPoints() {
        ...
        if (total_payable < total_paying) {
            ...
            addCoinPointsConfimation(balance);
        } else{
            sell_form.submit();
        }
    }

    function addCoinPointsConfimation(balance){
        swal({
            title: LANG.customer_coin_points,
            text: LANG.add_balance_to_customer_points,
            icon: 'info',
            buttons: true,
            dangerMode: true,
        }).then(confirm => {
            if (confirm) {
                $('#coin_points_earned_amount').val(parseFloat(balance));
                sell_form.submit();
            }else {
                sell_form.submit();
            }
...
function calculate_billing_details(price_total) {
    ...
    if ($('#enable_coin_points').length) {
        let total_customer_coin_points = $('#coin_points_redeemed_amount').val();
        discount = parseFloat(discount) + parseFloat(total_customer_coin_points);
        if ($('input[name="is_direct_sale"]').length <= 0) {
            $('span#total_discount').text(__currency_trans_from_en(discount, false));
        }
    }
    ...
    __write_number($('input#old_final_total_input'), total_payable_rounded);
    var curr_exchange_rate = 1;
...
function calculate_balance_due() {
    ...
    let old_total_payable = __read_number($('#old_final_total_input'));
    var total_paying = 0;
    $('#payment_rows_div')
        .find('.payment-amount')
        .each(function() {
    ...
    $('#payment_rows_div').find('.payment_types_dropdown').each(function () {
        if ($(this).val() === 'card'){
           ...
           let index = string.split('_')[1];
           let percentage = $('#card_holder_name_'+index).val();
           let added_value = (old_total_payable * percentage) / 100;
           let new_total = old_total_payable + added_value;
           total_payable = new_total;
           $('span.total_payable_span').text(__currency_trans_from_en(total_payable, true));
           $('#final_total_input').val(__number_f(total_payable));
        }else {
            total_payable = old_total_payable;
            $('span.total_payable_span').text(__currency_trans_from_en(old_total_payable, true));
            $('#final_total_input').val(__number_f(old_total_payable));
        }
    })
...
$(document).on('change', 'select#customer_id', function(){
    ...
    if ($(this).val() == default_customer_id) {
        //Disable reward points and coin points for walkin customers
        ...
        if ($('#coin_points_redeemed_amount_modal').length) {
            $('#coin_points_redeemed_amount_modal').val('');
            $('#coin_points_redeemed_amount_modal').change();
            $('#coin_points_redeemed_amount_modal').attr('disabled', true);
            $('#available_coin_points').text('');
            updateCoinPointsAmount();
            pos_total_row();
        }
    } else {
        ...
        if ($('#coin_points_redeemed_amount_modal').length) {
            $('#coin_points_redeemed_amount_modal').removeAttr('disabled');
        }
        getCustomerRewardPoints();
        getCustomerCoinPoints();
    }
});
...
// coin points
function getCustomerCoinPoints() {
    if ($('#enable_coin_points').length <= 0) {
        return false;
    }
    var is_edit = $('form#edit_sell_form').length ||
        $('form#edit_pos_sell_form').length ? true : false;
    ...
    $.ajax({
        method: 'GET',
        url: '/sells/pos/get-customer-coin-points',
        data: {
            customer_id: customer_id
        },
        dataType: 'json',
        success: function (result) {
            $('#available_coin_points').text(result.total_coins);
            $('#coin_points_redeemed_amount_modal').data('max_points', result.total_coins);
            updateCoinPointsAmount();
            $('#coin_points_redeemed_amount').change();
        },
    });
}

function updateCoinPointsAmount() {
    var points = $('#coin_points_redeemed_amount_modal').val().trim();
    points = points == '' ? 0 : parseInt(points);
    // $('#rp_redeemed_amount_text').text(__currency_trans_from_en(points, true));
    $('#coin_points_redeemed_amount').val(points);
}

$(document).on('change', '#coin_points_redeemed_amount_modal', function () {
    var points = $(this).val().trim();
    points = points == '' ? 0 : parseInt(points);
    var reward_validation = isValidCoinPoint();
    if (!reward_validation['is_valid']) {
        toastr.error(reward_validation['msg']);
        $('#coin_points_redeemed_amount_modal').select();
    }
});

$(document).on('change', '.direct_sell_coin_points_input', function () {
    updateCoinPointsAmount();
    pos_total_row();
});

function isValidCoinPoint() {
    var element = $('#coin_points_redeemed_amount_modal');
    var points = element.val().trim();
    points = points == '' ? 0 : parseInt(points);
    ...
    if (points > max_points) {
        is_valid = false;
        msg = __translate('max_coin_points_reached_error', { max_points: max_points});
    }
