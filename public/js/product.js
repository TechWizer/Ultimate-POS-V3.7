...
$(document).ready(function() {
    ...
    $(document).on('change', '.variation_template', function() {
        ...
        $(document).on('click', '#add_modal_button', function () {
            ...
            var values = [];
            $('#selectVariationValues :selected')
                .map(function (i, el) {
                    values.push($(el).val());
                })
                .get();
            ...
            $.ajax({
                ...
                dataType: 'html',
                data: { template_id: values, row_index: row_index },
                success: function (result) {
                    console.log(result);
                    if (result) {
        ...
        tr_obj.find('input.variation_name').val(row.find('option:selected').text());

        $.ajax({
            method: 'POST',
            url: '/products/get_variation_temp_values',
            dataType: 'html',
            data: { template_id: template_id, row_index: row_index },
            success: function (result) {
                console.log(result);
                if (result) {
                    $('#selectVariationValues').empty();
                    $.each(JSON.parse(result), function (key, value) {
                        $('#selectVariationValues').append(
